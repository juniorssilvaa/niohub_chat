    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-background border border-border w-full max-w-2xl rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {/* New Reminder Form */}
          <section>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Novo Lembrete</h3>
            <form onSubmit={handleSubmit} className="space-y-4 bg-muted/50 p-4 rounded-lg border border-border">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full p-2 bg-background border border-border rounded-lg focus:ring-1 focus:ring-border outline-none transition-all"
                    required
                  />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Mensagem / Anotação</label>
                <textarea
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  placeholder="Ex: Enviar orçamento para o cliente X..."
                  className="w-full p-2 bg-background border border-border rounded-lg focus:ring-2 focus:ring-primary outline-none min-h-[80px]"
                  required
                />
              </div>
              <button
          ...
          {/* List section */}
          <section>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Agendados</h3>
            {loading ? (
              ...
              <div className="space-y-3">
                {reminders.map(reminder => (
                  <div key={reminder.id} className={`p-4 rounded-lg border ${reminder.is_notified ? 'bg-muted/30 border-border opacity-60' : 'bg-background border-border shadow-sm'} flex items-start justify-between group`}>
