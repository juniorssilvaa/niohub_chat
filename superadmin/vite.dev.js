...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      port: 8013,
      strictPort: false,
      allowedHosts: ['chat.niohub.com.br', 'chat-local.niohub.com.br', 'localhost', '127.0.0.1'],
      cors: true,
      headers: {
      ...
      proxy: {
        '/api': { target: 'http://localhost:8014', changeOrigin: true, secure: false, ws: false },
        '/api-token-auth': { target: 'http://localhost:8014', changeOrigin: true, secure: false },
        '/auth': { target: 'http://localhost:8014/api', changeOrigin: true, secure: false },
        '/webhook': { target: 'http://localhost:8014', changeOrigin: true, secure: false },
        '/webhooks': { target: 'http://localhost:8014', changeOrigin: true, secure: false },
        '/media': { target: 'http://localhost:8014', changeOrigin: true, secure: false },
        '/ws': { target: 'ws://localhost:8014', ws: true, changeOrigin: true, secure: false },
      },
    },
  }
})
